...
\author{D. J. Haxton  \ \ C. W. McCurdy  \ \ T. N. Rescigno \ \ K. V. Lawler  \ \ B. Abeln \ \  X. Li \\
...
for the nonrelativistic Schrodinger equation.  Currently it uses the dipole approximation in length and velocity gauge and has options for a variety
...
\item{Vibronic wave functions diatoms using prolate coordinates (\verb#chmctdhf_diatom#)}
\item{Fixed nuclei wave functions for polyatomics without stretching or complex scaling (\verb#chmctdhf_sinc#)}
\end{itemize}
%
Future versions will support
\begin{itemize}[noitemsep]
\item{Electronic wave functions for atoms (\verb#chmctdhf_atom#)}
\item{Rovibronic wave functions diatoms using prolate coordinates (\verb#chmctdhf_diatom#)}
\item{Rovibronic wave functions diatoms using modified prolate coordinates (\verb#chmctdhf_diatom2#)}
...
\item{\textit{Perfect tensor decomposition for electronic structure using sinc basis functions.}
J. R. Jones, T. N. Rescigno, C. W. McCurdy,  K. Ibrahim, S. Williams, E. Vecharynski, F.-H. Rouet, X. Li, C. Yang, D. J. Haxton, in preparation.}        
        %
...
% SEVEN POSSIBLE: EARLY CAREER., CHG/SLAVA NSF; AMO BASE, UC DAVIS, LBNL GENERAL, NERSC GENERAL AS APPLICABLE.
%%%%%
%%%%%
...
There are three directories at top level, \verb#MCTDH.SRC#, \verb#EXAMPLES#, and \verb#COMPDIR#.  
...
There are many compilation directories ready-to-go for installation of the code on various machines, provided in \verb#COMPDIR#.  
...
directories in \verb#COMPDIR#, are 
\begin{itemize}
...
Either move the \verb#BIN# directories that you are interested in compiling to top level, from \verb#COMPDIR#, or go into \verb#COMPDIR# to compile them.
You can compile either in top level or \verb#COMPDIR#; the symbolic links to \verb#MCTDH.SRC# will work either way.
You may delete the directories in \verb#COMPDIR# that you don't want, but they don't take too much space.

All you need to do is go to the \verb#BIN# directory you want and run \verb#./Makeme#.  The code should then be compiled as, e.g.
{\footnotesize
\begin{verbatim}
prompt> mv COMPDIR/*BIN*mac* .
prompt> rm -r COMPDIR
prompt> cd BIN.ecs.hermnorm.mac
prompt> ./Makeme
prompt> ls
      .
      .
...
prompt> cd ~/bin
prompt> ln -s ~/myprogs/LBNL-AMO-MCTDHF/MCTDHF.010115.VERSION1.0/BIN.ecs.hermnorm/chmctdhf_diatom ./chmctdhf_diatom
prompt> ln -s ~/myprogs/LBNL-AMO-MCTDHF/MCTDHF.010115.VERSION1.0/debug.BIN/mctdhf_sinc ./mctdhf_sinc.debug
\end{verbatim}}
etc.
...
%
\item{\verb#improvedquadflag=1# is the option for doing Newton solve for the A-vector to converge excited states.  It used to be 2.  Values other than 0 or 1 are disabled and will result in an 
error.}  
...
%Don't waste your time running bigger primitive basis calculations than are necessary.  Check the timing output to see where and why the calculation is running slow.  If necessary increase \verb#maxexpodim# or \verb#maxaorder# and perhaps \verb#expodim# or \verb#aorder#.  \textbf{Restarts of Krylov iterations are okay for propagation}; only bad for \verb#improvedquadflag#=1 improved relaxation.  But they do make it slower, so increase the maximum dimensions to several hundred and you should get the best performance.
...
compilation directories provided for the lawrencium machine in \verb#COMPDIR#.  The sequential (no shared memory, only
...
%
%\item{\textbf{For excited state convergence (}\verb#improvedquadflag#\textbf{=1) we recommend performing a run with} \verb#spf_flag#=0 \textbf{and using it as a guess for one %with} \verb#spf_flag#=1.  Remember to use a sufficient \verb#aorder#.  It will give a clear warning if the Krylov iterations are restarting, which in this case is bad.}
\end{itemize}
...
the reduced operator $\mathbf{W}=tr(\Gamma\mathbf{w})$ (NOTATION??) 
$\mathbf{W}_{\alpha\beta}=\Gamma_{\alpha\beta}^{\gamma\delta}\mathbf{w}_{\gamma\delta}$
(????)
...
There are many directories ready-to-go for installation of the code on various machines, provided in \verb#COMPDIR#.  
There are optimized versions, and versions with debugging flags.  Please compile the debugging version too and use it if the code
seems to be producing an invalid or unstable result, or actually crashes.  The supported machines, the machines with ready-to-go
directories in \verb#COMPDIR#, are 
\begin{itemize}
...
Either move the \verb#BIN# directories that you are interested in compiling to top level, from \verb#COMPDIR#, or go into \verb#COMPDIR# to compile them.
You can compile either in top level or \verb#COMPDIR#; the symbolic links to \verb#MCTDH.SRC# will work either way.
You may delete the directories in \verb#COMPDIR# that you don't want, but they don't take any space.
...
prompt> echo $PATH
  /usr/bin:/usr/sbin:/opt/local/bin:/home/me/bin
prompt> cd ~/bin
prompt> ln -s myprogs/LBNL-AMO-MCTDHF/MCTDHF.010115.VERSION1.0/BIN.ecs.hermnorm.mac/chmctdhf_diatom ./chmctdhf_diatom
prompt> ln -s myprogs/LBNL-AMO-MCTDHF/MCTDHF.010115.VERSION1.0/debug.BIN.ecs.hermnorm.mac/chmctdhf_diatom ./chmctdhf_diatom.debug
\end{verbatim}}
...
an excited state is desired, by inverse iterations using \verb#improvedquadflag=1#.
...
\verb#improvedquadflag=1#.  All other values of improvedrelaxflag are deprecated at the moment.}
\end{itemize}
...
The default is to use core orbitals and eigenfunctions.  However, one may wish to read in an arbitrary set of initial wave functions.  For instance, one may wish to converge the first five states including an excited $\Pi$ state using relaxation, then select one component of that $\Pi$ state and use \verb#improvedquadflag=1# to iteratively refine that state.
...
Because the
lowest states aren't being calculated, \verb#improvedquadflag=1# must be invoked, otherwise the 3rd sigma state will collapse to the 1st.
...
Again, the directory structure, right out of the package, is as follows.  One may move the desired \verb#BIN# directories to top level and remove \verb#COMPDIR#
entirely.
